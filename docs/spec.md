...
```
┌─────────────┐     ┌──────────────────────┐     ┌─────────────┐
│  UD-CO2S    │────▶│ chissoku（バイナリ）  │────▶│  forwarder  │
│  (USB)      │     │ 子プロセスで起動      │     │  (ホスト)   │
└─────────────┘     └──────────────────────┘     └──────┬──────┘
                                                  │
                                                  ▼
...
| chissoku | センサからデータを読み取り JSON を stdout に出す（**Linux 用バイナリ**を `CHISSOKU_BIN` で指定） |
| forwarder | chissoku を子プロセスとして起動し stdout を読み、Mackerel に送信 |
...
1. **chissoku の起動**: `CHISSOKU_BIN` で指定した実行ファイルを子プロセスとして起動する
...
- forwarder 起動時に chissoku を子プロセスとして起動
- chissoku が終了した場合は再起動
...
| 引数 | 説明 |
|------|------|
| `--dry-run` | chissoku からセンサ値を読み、**パースして表示するだけ**。Mackerel には **POST しない**（`MACKEREL_API_KEY` があっても送らない）。 |

### 5.1 環境変数

| 変数名 | 説明 | 必須 | 未設定時の挙動 |
|--------|------|------|----------------|
| `CHISSOKU_BIN` | chissoku の Linux 用実行ファイルのパス（例: `/opt/chissoku/chissoku`） | **はい** | 起動に失敗する |
